/* =========================================================
   main.js — FINAL (modal & keyboard fixed)
   ========================================================= */
...
const parse = l => {
  const m=l.match(/^<(\w+)>/);
  return m?{content:l.slice(m[0].length).trimStart(),tag:m[1]}:{content:l,tag:null};
};
...
document.addEventListener("DOMContentLoaded",async()=>{
  ...
  const savedName = await (async()=>{
    try{const r=await fetch("/api/whoami");return (await r.json()).name}catch{return null}
  })();
  ...
  const highlightMenu=i=>{
    menuItems.forEach((li,x)=>li.querySelector(".icon-btn")
      .classList.toggle("selected",x===i));
    menuIdx=i;
  };
  ...
  function spawnMainChoices(force = false){
    ...
    mainIdx=0; $choices.innerHTML="";
    const wrap=ce("div","choices");
    const g=mkBtn("Tamu","guest",true),
          n=mkBtn("Isi Nama","name");
    wrap.append(g,n); $choices.append(wrap);
    mainBtns=[g,n]; setMain(0);
  ...
  function spawnSubChoices(yesCB,noCB,yesT="Tentu",noT="Tidak"){
    ...
    const w=ce("div","choices"),
          y=mkBtn(yesT,"yes",true),
          n=mkBtn(noT,"no");
    w.append(y,n); $choices.append(w);
    subBtns=[y,n]; setSub(0);
  ...
  const doProyekMoreYes = ()=>{
    q=["<askProjPrompt>Mau Lihat yang Mana?"];idx=pos=0;type();
  };
    ...
    btn.onclick = async ()=>{
      ...
      if(path==="/proyek"){
        ...
        if(!Array.isArray(intro)){
          console.warn("dialog_proyek intro invalid:",intro);
